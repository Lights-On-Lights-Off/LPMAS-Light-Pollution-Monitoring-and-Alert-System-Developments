...
const features = [
  ...
  { icon: <ShieldCheck strokeWidth={1.75} />, title: "Role-based access", text: "Admins and managers, each scoped to what they need." },
  ...
  return <main className="theme-transition w-full max-w-full overflow-x-clip bg-ink font-sans text-metal-100 antialiased">
    <header className="sticky top-0 z-30 border-b border-metal-700 bg-ink/80 backdrop-blur-xl">
      <nav className="mx-auto flex h-[72px] max-w-6xl items-center justify-between px-5">
        ...
        <div className="hidden items-center gap-8 text-sm text-metal-300 md:flex">
          ...
          <Link href="/appearance" className="transition hover:text-metal-50">Appearance</Link>
        ...
        <button onClick={() => setMenuOpen(!menuOpen)} className="rounded-xl p-2 transition hover:bg-theme-surface-hover md:hidden" aria-label="Toggle menu">{menuOpen ? <X /> : <Menu />}</button>
      </nav>
      {menuOpen && <div className="space-y-1 border-t border-metal-700 px-5 py-4 md:hidden">
        ...
        <Link href="/appearance" onClick={() => setMenuOpen(false)} className="block py-2">Appearance</Link>
        <Link href="/login" onClick={() => setMenuOpen(false)} className="mt-2 block rounded-full bg-leaf-500 px-5 py-3 text-center font-semibold text-ink">Sign in</Link>
      </div>}
    </header>
      ...
      <div className="relative mx-auto grid max-w-6xl items-center gap-14 lg:grid-cols-[1.05fr_.95fr]">
        <motion.div initial={reduce ? undefined : { opacity: 0, y: 16 }} animate={reduce ? undefined : { opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: "easeOut" }}>
          ...
          <div className="mt-9 flex flex-wrap gap-3">
            ...
            <Link href="/monitor" className="rounded-full border border-metal-600 px-7 py-3.5 font-semibold transition hover:bg-theme-surface-hover active:scale-[0.98]">View live data</Link>
          </div>
        </motion.div>
    ...
    <section id="features" className="mx-auto max-w-6xl px-5 py-24 md:py-32">
      ...
      <div className="mt-12 grid gap-4 md:grid-cols-2">
        {features.map(f => <motion.article key={f.title} {...fade} whileHover={reduce ? undefined : { y: -3 }} className={`rounded-2xl border border-metal-700 bg-metal-800/60 p-6 ${f.wide ? "md:col-span-2" : ""}`}>
          <div className={f.wide ? "flex items-start gap-6" : ""}>
            ...
            <div className={f.wide ? "flex-1" : ""}>
              <h3 className="mt-6 text-lg font-bold sm:mt-0">{f.title}</h3>
    ...
    <section id="about" className="mx-auto max-w-6xl px-5 py-24 md:py-32">
      <div className="overflow-hidden rounded-3xl border border-metal-700 bg-gradient-to-br from-leaf-900 to-ink px-8 py-14 md:px-16">
        <div className="grid items-center gap-10 md:grid-cols-[1fr_auto]">
          ...
          <Link href="/login" className="inline-flex items-center justify-center gap-2 rounded-full bg-leaf-500 px-7 py-3.5 font-semibold text-ink transition hover:bg-leaf-100 active:scale-[0.98]">Sign in <ArrowRight size={18} /></Link>
        </div>
      </div>
    </section>
